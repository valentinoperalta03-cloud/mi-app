...
import { ChevronLeft, ChevronRight, Dumbbell, GraduationCap } from "lucide-react";
...
export type ProfessorGroup = { key: string; professorName: string; schedules: ProfessorSchedule[] };
...
export default function ClasesHubClient({
  ...
  if (view === "entrenamientos") {
    return (
      <div className="flex flex-col gap-6">
        <HubBackButton onClick={() => setView("hub")} />
        <AdminPageHeader
          ...
          {punctualItems.map((item) => (
            <li key={item.key} className={`${adminCard} flex items-start justify-between gap-3`}>
              ...
              <form action={deleteExternalTrainingBlockRowAction}>
                <input type="hidden" name="court_block_id" value={item.courtBlockId} />
                <button type="submit" className={adminCTADangerCompact}>
                  Dar de baja
                </button>
              </form>
    ...
    return (
      <div className="flex flex-col gap-6">
        ...
        <AdminPageHeader
          ...
          action={<ClasePublicaModal clubId={clubId} courts={courtOptions} coaches={coachOptions} />}
        />
              ...
              <li key={p.id}>
                ...
                >
                  ...
                  <p className="mt-1 text-xs text-[var(--text-tertiary)]">
                    ...
                    {p.sessionCount > 0 ? ` · ${p.sessionCount} fecha(s)` : ""} · {p.max_spots} cupos · $
  ...
  return (
    <div className="flex flex-col gap-6">
      <AdminPageHeader kicker="Gestión de juego" title="Clases y entrenamientos" subtitle="Elegí qué querés gestionar" />
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        >
          ...
          <div>
            <p className="text-base font-bold text-[var(--text-primary)]">Entrenamientos externos</p>
            <p className="mt-1 text-sm leading-relaxed text-[var(--text-secondary)]">
              Profesores particulares que usan tus canchas. Bloquean el horario automáticamente.
        ...
        >
          ...
          <div>
            <p className="text-base font-bold text-[var(--text-primary)]">Clases públicas</p>
            <p className="mt-1 text-sm leading-relaxed text-[var(--text-secondary)]">
              Clases que publicás para que los jugadores se inscriban y paguen desde la app.
              Tienen precio, cupos y horarios visibles.
            </p>
          </div>
