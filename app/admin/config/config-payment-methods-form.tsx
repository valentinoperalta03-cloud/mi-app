...
import { adminPressable } from "@/components/admin/admin-premium";
    ...
    <form action={savePaymentMethods} className="space-y-4">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          >
            Mercado Pago · {isMpConnected ? "Conectado" : "Desconectado"}
          </p>
        </div>
      ...
      <label className="block space-y-1">
        <span className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Alias</span>
        <input name="bank_alias" defaultValue={initial.bank_alias} placeholder="Ej. mi.alias.mp" className={inputClass} />
      </label>
      <label className="block space-y-1">
        <span className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">CBU (opcional)</span>
