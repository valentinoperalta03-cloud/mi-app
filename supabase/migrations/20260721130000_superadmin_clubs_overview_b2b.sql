-- Actualiza superadmin_clubs_overview al modelo B2B: saca el CTE de
-- club_debts (modelo de comisiones discontinuado, ver
-- app/superadmin/(main)/finanzas/page.tsx antes de este cambio) y agrega
-- las columnas de suscripcion mensual ($50.000 ARS fijos por club).
--
-- subscription_status/trial_end_date/next_billing_date/mp_subscription_id
-- estan revocadas para anon/authenticated (20260713120000_b2b_saas_...),
-- por eso esta vista sigue el mismo patron que ya tenia (sin
-- security_invoker, grant solo a service_role): el owner de la vista lee
-- las columnas protegidas y unicamente service_role puede consultarla.
