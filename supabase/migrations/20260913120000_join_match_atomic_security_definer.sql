-- join_match_atomic era security invoker: el SELECT ... FOR UPDATE sobre
-- matches queda sujeto no solo a la policy de SELECT ("Partidos visibles
-- para todos", qual = true) sino tambien a la policy de UPDATE de esa tabla
-- (auth.uid() = owner_id), porque Postgres RLS exige que una fila bloqueada
-- con FOR UPDATE pase tambien la policy de UPDATE aplicable. Resultado: un
-- jugador que no es el dueno del club nunca encontraba la fila (match_not_found)
-- y el join fallaba siempre con "Algo salio mal al guardar" (join_error=db),
-- salvo que quien llamara fuera el dueno.
--
-- Se cambia a security definer: la funcion ya revalida manualmente
-- auth.uid() = p_player_id o auth.uid() = owner_id (o null para service_role,
-- ver migracion 20260801120000), asi que definer no abre ningun permiso
-- nuevo, solo evita que el candado FOR UPDATE choque con la policy de UPDATE.
-- Mismo patron que confirm_participant_payment_atomic en este mismo archivo
-- original.
