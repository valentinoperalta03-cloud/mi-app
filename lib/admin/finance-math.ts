...
export function totalPaid(rows: MatchMoneyRow[]) {
  return filterPaid(rows).reduce((s, r) => s + Number(r.total_price ?? 0), 0);
}
...
export function aggregateByWeek(rows: MatchMoneyRow[]) {
  ...
  for (const r of paid) {
    ...
    map.set(wk, (map.get(wk) ?? 0) + Number(r.total_price ?? 0));
  }
...
export function aggregateByMonth(rows: MatchMoneyRow[]) {
  ...
  for (const r of paid) {
    ...
    map.set(mk, (map.get(mk) ?? 0) + Number(r.total_price ?? 0));
  }
  return Array.from(map.entries())
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([k, total]) => ({
