 ...
 * partido hijo: si ese hijo ya se jugó con la pareja vieja, su resultado
 * queda basado en un cruce que nunca existió, así que hay que invalidarlo en
 * cascada en vez de pisarle la pareja en silencio.
 */
...
export async function propagateBracket(admin: SupabaseClient, finishedMatchId: string, winnerPairId: string | null) {
  ...
  for (const row of (children ?? []) as Array<{
    ...
    // de pisarla — evita dejar un resultado aplicado sobre un cruce que ya
    // no existe.
