...
export async function HomeReservationsSection({ userId }: { userId: string }) {
  ...
  if (list.length === 0) {
    return (
      <div className="rounded-[2rem] border border-dashed border-slate-200/90 bg-white/80 px-4 py-6 text-center">
        ...
        <Link
          href="/buscar-partido"
          className="mt-4 inline-flex items-center justify-center rounded-2xl bg-sky-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-sky-500"
        >
          Buscar Partido
        </Link>
      </div>
  ...
  return (
    <ul className="flex flex-col gap-3">
      {list.map((r) => (
        <li key={r.matchId}>
          <article className="rounded-[2rem] border border-slate-200/80 bg-white p-5 shadow-[0_2px_12px_-4px_rgba(15,23,42,0.08)]">
              ...
              <div className="min-w-0 flex-1 space-y-1.5">
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 pt-1">
                  <Link
                    ...
                    className="text-xs font-semibold text-sky-700 underline decoration-sky-200/80 underline-offset-2 hover:text-sky-800"
                  >
                    Ver detalle
                  </Link>
